        ...
        def __getitem__(self,idx):
            ...
            encoder_input = torch.cat(
                [
                  ...
                  self.eos_token,
                  torch.tensors([self.pad_token]*enc_num_padding_tokens,dtype=torch.int64),
                ],
                dim=0,
            )
            ...
            decoder_input = torch.cat(
                [
                  ...
                  torch.tensors([self.pad_token]*dec_num_padding_tokens,dtype=torch.int64),
                ],
                dim=0,
            )
            ...
            return {
                ...
                "encoder_mask":(encoder_input!=self.pad_tokens).unsqueeze(0).unsqueeze(0).int(),
                "decoder_mask":(decoder_input!=self.pad_tokens).unsqueeze(0).int() & casual_mask(decoder_input.size(0)),
                "src_text":src_text,
                "tgt_text":tgt_text,
